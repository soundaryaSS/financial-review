(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))c(s);new MutationObserver(s=>{for(const o of s)if(o.type==="childList")for(const i of o.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&c(i)}).observe(document,{childList:!0,subtree:!0});function t(s){const o={};return s.integrity&&(o.integrity=s.integrity),s.referrerPolicy&&(o.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?o.credentials="include":s.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function c(s){if(s.ep)return;s.ep=!0;const o=t(s);fetch(s.href,o)}})();const I="http://localhost:8000/api";async function be(){const e=await fetch(`${I}/pnl`);if(!e.ok)throw new Error("Failed to load P&L Statement");return e.json()}async function fe(e="2026-02",n="2026-03"){const t=await fetch(`${I}/variances?period_a=${e}&period_b=${n}`);if(!t.ok)throw new Error("Failed to load Variance Analysis");return t.json()}async function ge(){const e=await fetch(`${I}/review-queue`);if(!e.ok)throw new Error("Failed to load Review Queue");return e.json()}async function ve(e){const n=new URLSearchParams,t=await fetch(`${I}/transactions?${n.toString()}`);if(!t.ok)throw new Error("Failed to load Transactions");return t.json()}async function Q(e,n){const t=await fetch(`${I}/transactions/${e}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(n)});if(!t.ok)throw new Error(`Failed to update transaction ${e}`);return t.json()}async function ye(e,n=[]){const t=await fetch(`${I}/chat`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({message:e,conversation_history:n})});if(!t.ok)throw new Error("Failed to get answer from AI Analyst");return t.json()}async function he(e){const n=new FormData;n.append("file",e);const t=await fetch(`${I}/upload`,{method:"POST",body:n});if(!t.ok)throw new Error("Failed to upload CSV file");return t.json()}async function we(){const e=await fetch(`${I}/reset`,{method:"POST"});if(!e.ok)throw new Error("Failed to reset dataset");return e.json()}function m(e){return new Intl.NumberFormat("en-US",{style:"currency",currency:"USD",minimumFractionDigits:2,maximumFractionDigits:2}).format(e)}function X(e){return`${e>0?"+":""}${e.toFixed(1)}%`}function K(e){const n=Math.round(e*100);return e>=.9?`<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">${n}% Confident</span>`:e>=.7?`<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-500/10 text-amber-400 border border-amber-500/20">${n}% Uncertain</span>`:`<span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-rose-500/10 text-rose-400 border border-rose-500/20">${n}% Review Req.</span>`}function $e(e){return e==="high"?'<span class="px-2 py-0.5 text-xs font-semibold rounded bg-rose-500/20 text-rose-300 border border-rose-500/40">HIGH ATTENTION</span>':e==="medium"?'<span class="px-2 py-0.5 text-xs font-semibold rounded bg-amber-500/20 text-amber-300 border border-amber-500/40">MEDIUM REVIEW</span>':'<span class="px-2 py-0.5 text-xs font-semibold rounded bg-blue-500/20 text-blue-300 border border-blue-500/40">LOW JUDGMENT</span>'}function Ee(e,n){const t=e.periods,c=t.map(r=>`<th class="py-3 px-4 text-right font-semibold text-slate-300 uppercase tracking-wider text-xs">${r}</th>`).join("");function s(r,d,y){return r.length?r.map(v=>{const h=t.map(E=>{const S=v.monthly_amounts[E]||0,k=S<0||v.name.toLowerCase().includes("refund");return`
  ...
  `}let w="pnl",L=null,z=[],ie=[],$=[],T=[],D=!1,B="",P="",j="",O="",F="",Z="",ee="",_=null;const f=document.getElementById("view-container"),Te=document.getElementById("review-count-badge"),de=document.getElementById("audit-drawer"),N=document.getElementById("drawer-backdrop"),G=document.getElementById("close-drawer-btn"),Ce=document.getElementById("drawer-title"),_e=document.getElementById("drawer-subtitle"),H=document.getElementById("drawer-tx-list"),ke=document.getElementById("drawer-total-amount"),ce=document.getElementById("edit-modal"),V=document.getElementById("modal-backdrop"),q=document.getElementById("close-modal-btn"),W=document.getElementById("cancel-modal-btn"),U=document.getElementById("save-modal-btn"),Be=document.getElementById("modal-tx-desc"),Pe=document.getElementById("modal-tx-meta"),pe=document.getElementById("modal-cat-select"),xe=document.getElementById("modal-subcat-input"),me=document.getElementById("modal-pnl-select"),ue=document.getElementById("modal-notes-input"),te=document.getElementById("toast"),je=document.getElementById("toast-message");function C(e){je.textContent=e,te.classList.remove("translate-y-20","opacity-0"),setTimeout(()=>{te.classList.add("translate-y-20","opacity-0")},3500)}async function A(){var e;try{const[n,t,c,s]=await Promise.all([be(),fe("2026-02","2026-03"),ge(),ve()]);L=n,z=t,ie=c,$=s,Te.textContent=String(c.length),g()}catch(n){console.error("Failed to load data from backend:",n),f.innerHTML=`
    ...
    `,(e=document.getElementById("retry-btn"))==null||e.addEventListener("click",A)}}function R(e,n,t){Ce.textContent=e,_e.textContent=n;const c=t.reduce((s,o)=>s+o.amount,0);ke.textContent=m(c),t.length?H.innerHTML=t.map(s=>`
