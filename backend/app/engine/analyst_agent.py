...
def answer_financial_query_deterministic(
    ...
    if "revenue" in q or "sales" in q or "toast" in q or "income" in q or "turnover" in q:
        ...
        answer = (
            ...
            f"**Monthly Breakdown:**\n"
            f"- **January 2026:** ${(m_jan.revenue if m_jan else 0.0):,.2f}\n"
            f"- **February 2026:** ${(m_feb.revenue if m_feb else 0.0):,.2f} (-11.5% due to 28-day month & winter storm)\n"
            f"- **March 2026:** **${(m_mar.revenue if m_mar else 0.0):,.2f}** (+27.2% surge)\n\n"
            f"**Revenue Channels:**\n"
